...
extern "C++" {
inline float smoothstep(const float t1, const float t2, const float x) {
    if (x <= t1) return 0.f;
    if (x >= t2) return 1.f;
    float k = (x - t1) / (t2 - t1);
...
inline float gamma_correction(const float val, const float gamma) {
    return std::pow(val, gamma);
}

inline vec3 gamma_correction(const vec3 &val, const float gamma) {
